...
Read `AGENTS.md`, `docs/plans/mvp-demo-plan.md`, `docs/loop/ACCEPTANCE.md`,
`docs/loop/RESEARCH.md`, `docs/loop/tasks.json`, `docs/loop/task-status.json`,
and the slice contract. Do not edit files.
...
4. The task profile's specialized gate is satisfied.
5. Public docs do not overclaim beyond the local synthetic demo.
6. `scripts/loop/verify.sh` is green, or the remaining failure is expected for
   the harness-only phase.
7. Greptile gate is present for PR workflows.
